...
- **LLMService**: Un service résilient pour interagir avec les grands modèles de langage (LLM). Il intègre une logique de relance (retry) pour gérer les erreurs de parsing et les appels réseau.
...
Le `LLMService` a été conçu pour être résilient aux erreurs de parsing et aux problèmes de réseau lors de l'appel aux LLM. Il utilise une stratégie de relance avec un backoff exponentiel pour réessayer les appels qui échouent. Si une opération échoue après 3 tentatives, la tâche est abandonnée gracieusement pour éviter de bloquer le système.

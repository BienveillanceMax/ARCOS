...
public class MoodIntegrationTest {
    ...
    void testMoodPromptGeneration() {
        // 1. Verify Prompt Builder includes mood info
        Prompt prompt = promptBuilder.buildConversationnalPrompt(context, "Hello");
        String promptContent = prompt.toString();

        // The prompt should contain personality and mood context, but not necessarily JSON instructions anymore
        // as the architecture seems to have moved to streaming text response + separate async mood update.
